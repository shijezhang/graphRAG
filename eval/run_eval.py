...
JUDGE_SYSTEM = """You are an expert evaluator for RAG systems. Given a question, a reference \
answer, and a system's answer, score the system answer from 0 to 10.
...
def run_evaluation():
    ...
    for q in questions:
        ...
        results.append(
            EvalResult(qid, qtype, question, "BM25", bm25_answer, bm25_kc, bm25_score, bm25_reason, bm25_latency)
        )
        ...
        results.append(
            EvalResult(qid, qtype, question, "GraphLocal", gl_answer, gl_kc, gl_score, gl_reason, gl_latency)
        )
        ...
        results.append(
            EvalResult(qid, qtype, question, "GraphGlobal", gg_answer, gg_kc, gg_score, gg_reason, gg_latency)
        )
    ...
    Path("eval/results/detailed.json").write_text(json.dumps(results_data, ensure_ascii=False, indent=2))
...
def print_summary(results: list[EvalResult], questions: list[dict]):
    ...
    for q in questions:
        ...
        bm25_s = scores.get("BM25", 0)
        gl_s = scores.get("GraphLocal", 0)
        gg_s = scores.get("GraphGlobal", 0)
        print(f"{qid:<5} {q['type']:<7} {bm25_s:>6.0f} {gl_s:>8.0f} {gg_s:>8.0f}")
